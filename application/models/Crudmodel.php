...
class Crudmodel extends CI_Model {
    ...
    function list_report($table, $numPage, $offset, $dawal, $akhir, $chapter, $jabatan) {
        if ($dawal == $akhir) {
            $this->db->where('aktifdate', $dawal);
        } else {
            $accommodation = 'aktifdate';
            $this->db->where("$accommodation BETWEEN $dawal AND $akhir");
        }
        if ($chapter != 0) {
            $this->db->where('idchapter', $chapter);
        }
        if ($jabatan != 0) {
    ...
    function list_reportregul($table, $numPage, $offset, $dawal, $akhir, $chapter) {
        if ($dawal == $akhir) {
            $this->db->where('insertdate', $dawal);
        } else {
            $accommodation = 'insertdate';
            $this->db->where("$accommodation BETWEEN $dawal AND $akhir");
        }
        if ($chapter != 0) {
            $this->db->where('idchapter', $chapter);
    ...
    function list_reportinpro($table, $numPage, $offset, $dawal, $akhir, $chapter, $jabatan) {
        if ($dawal == $akhir) {
            $this->db->where('waktu_ambil', $dawal);
        } else {
            $accommodation = 'waktu_ambil';
            $this->db->where("$accommodation BETWEEN $dawal AND $akhir");
        }
        if ($kurir != 0) {
        ...
        if ($shipp != 0) {
            $this->db->where('id_statusshipp', $shipp);
        }
        $this->db->limit($numPage, $offset);
        return $this->db->get($table);
    }
    
    function list_reportinpro2($table, $numPage, $offset, $dawal, $akhir, $kurir, $shipp) {
        if ($dawal == $akhir) {
            $this->db->where('waktu_diterima', $dawal);
        } else {
    ...
    function list_reportinpro3($table, $numPage, $offset, $dawal, $akhir, $kurir, $shipp) {
        if ($dawal == $akhir) {
            $this->db->where('waktu_dikembalikan', $dawal);
        } else {
    ...
    function list_retoday($table, $numPage, $offset, $dawal, $akhir, $kurir, $shipp) {
        $accommodation = 'aktifdate';
        $this->db->where("$accommodation BETWEEN $dawal AND $akhir");
//        if ($kurir != 0) {
//            $this->db->where('id_kurir', $kurir);
...
//        }
    ...
    function list_today($table, $dawal, $akhir, $shipp) {
//        if ( $dawal != 0){
        $accommodation = 'aktifdate';
        $this->db->where("$accommodation BETWEEN $dawal AND $akhir");
//        }
//        $this->db->where('tgl_upload', $tgltoday);
//        $this->db->where('id_statusshipp', $shipp);
        return $this->db->get($table);
    }
    
    function list_inpro($table, $shipp) {
//        if ( $dawal != 0){
//        $this->db->where('tgl_upload', $tgltoday);
//        $this->db->where('id_statusshipp');
        return $this->db->get($table);
    }

    function list_view($table, $numPage, $offset, $finds, $findt) {
        $this->db->order_by('nopung', 'asc');
        if ($findt != '') {
            $this->db->like($finds, $findt);
        }
        if( $this->session->userdata('idrool') > 2){
            $this->db->where('idchapter', $this->session->userdata('idchapter'));
        }
        $this->db->limit($numPage, $offset);
        return $this->db->get($table);
    }

    function list_data($table, $numPage, $offset, $finds, $findt) {
    ...
    function count_report($table, $dawal, $akhir, $chapter, $jabatan) {
        if ($dawal == $akhir) {
            $this->db->where('aktifdate', $dawal);
        } else {
            $accommodation = 'aktifdate';
        ...
        return $this->db->get($table);
//        return $this->db->count_all('v_'.$table);
    }

    function count_reportregul($table, $dawal, $akhir, $chapter) {
        if ($dawal == $akhir) {
            $this->db->where('insertdate', $dawal);
        } else {
            $accommodation = 'insertdate';
...
//        return $this->db->count_all('v_'.$table);
    ...
    function count_reportinpro($table, $dawal, $akhir, $kurir, $shipp) {
        if ($dawal == $akhir) {
            $this->db->where('waktu_ambil', $dawal);
        } else {
            $accommodation = 'waktu_ambil';
...
//        return $this->db->count_all('v_'.$table);
    ...
    function count_reportinpro2($table, $dawal, $akhir, $kurir, $shipp) {
        if ($dawal == $akhir) {
            $this->db->where('waktu_diterima', $dawal);
        } else {
            $accommodation = 'waktu_diterima';
...
//        return $this->db->count_all('v_'.$table);
    ...
    function count_reportinpro3($table, $dawal, $akhir, $kurir, $shipp) {
        if ($dawal == $akhir) {
            $this->db->where('waktu_dikembalikan', $dawal);
        } else {
            $accommodation = 'waktu_dikembalikan';
...
//        return $this->db->count_all('v_'.$table);
    ...
    function count_retoday($table, $dawal, $akhir, $kurir, $shipp) {
        $accommodation = 'aktifdate';
        $this->db->where("$accommodation BETWEEN $dawal AND $akhir");
//        if ($kurir != 0) {
//            $this->db->where('id_kurir', $kurir);
...
//        return $this->db->count_all('v_'.$table);
    ...
    function getKurirName($idkurir) {
        $this->db->where('id_chapter', $idkurir);
        $this->db->limit(1);
        return $this->db->get('v_chapter')->row()->namachapter;
    }

    function getShippKet($idshipp) {
        $this->db->where('id_jabatan', $idshipp);
        $this->db->limit(1);
        return $this->db->get('v_jabatan')->row()->nama;
    }
    ...
    function get_view_by_id($table, $id) {
        $this->db->where('id_' . $table, $id);
        return $this->db->get('v_' . $table);
    }
    ...
    function file_view($type, $id, $width = '100', $height = '100', $thumb = 'no', $src = 'no', $multi = '', $multi_num = '', $ext = '.jpg') {
        if ($multi == '') {
            ...
            $num = $this->crudmodel->get_type_name_by_id($type, $id, 'num_of_imgs');
            //$num = 2;
            $i = 0;
            $p = 0;
            $q = 0;
    ...
    function product_link($product_id, $quick = '') {
        if ($quick == 'quick') {
            ...
            $name = url_title($this->crudmodel->get_type_name_by_id('product', $product_id, 'title'));
    ...
    function blog_link($blog_id) {
        $name = url_title($this->crudmodel->get_type_name_by_id('blog', $blog_id, 'title'));
        return base_url() . 'index.php/home/blog_view/' . $blog_id . '/' . $name;
    }

    //GET PRODUCT LINK
    function vendor_link($vendor_id) {
        $name = url_title($this->crudmodel->get_type_name_by_id('vendor', $vendor_id, 'display_name'));
            ...
            foreach ($all as $row):
                if ($type == 'add') {
                    ...
                    if ($condition == '') {
                        ...
                        $return .= $this->crudmodel->get_type_name_by_id($condition, $row, $c_match);
                    }
                    $return .= '</option>';
                } else if ($type == 'edit') {
                    $return .= '<option value="' . $row . '" ';
    ...
    function get_additional_fields($product_id) {
        $additional_fields = $this->crudmodel->get_type_name_by_id('product', $product_id, 'additional_fields');
    ...
    function decrease_quantity($product_id, $quantity, $sale_id = '') {
        $prev_quantity = $this->crudmodel->get_type_name_by_id('product', $product_id, 'current_stock');
    ...
    function get_brands($type, $id, $vendor = '') {
        if ($type == 'category') {
            ...
            $sub_cats = array(array('id' => $id));
        }
        $brands = array();

        foreach ($sub_cats as $row) {
            $n_brands = json_decode($this->db->get_where('sub_category', array('id' => $row['id']))->row()->brand);
            foreach ($n_brands as $n) {
                if ($vendor !== '') {
                    if ($this->is_brand_of_vendor($n, $vendor)) {
                        $na = $n;
                        $na .= ':::';
                        $brn_data = $this->db->get_where('brand', array('id' => $n));
    ...
    function sub_details_by_cat($cat = '') {
        ...
        foreach ($subs as $row) {
            $result[] = array(
                'sub_id' => $row['id'],
                'sub_name' => str_replace("'", ' ', $row['name']),
                'min' => round($this->crudmodel->get_range_lvl('sub_category', $row['id'], "min")),
                'max' => round($this->crudmodel->get_range_lvl('sub_category', $row['id'], "max")),
                'brands' => str_replace("'", ' ', join(';;;;;;', $this->get_brands('sub_category', $row['id'])))
            );
        }
        ...
        foreach ($vendors as $row) {
            if ($type == 'category') {
                if ($id == '0') {
                    ...
                    if ($this->is_category_of_vendor($id, $row['id'])) {
                        $result[] = $row['id'] . ':::' . $row['display_name'];
                    }
                }
            }
            if ($type == 'sub_category') {
                if ($this->is_sub_cat_of_vendor($id, $row['id'])) {
                    $result[] = $row['id'] . ':::' . $row['display_name'];
                }
            }
        }
    ...
    function admin_permission($codename) {
        ...
        if ($admin->role == 1) {
            ...
            $role = $admin->role;
            $role_permissions = json_decode($this->crudmodel->get_type_name_by_id('role', $role, 'permission'));
    ...
    function get_range_lvl($by = "", $id = "", $type = "") {
        $physical = $this->crudmodel->get_settings_value('general_settings', 'physical_product_activation');
        $digital = $this->crudmodel->get_settings_value('general_settings', 'digital_product_activation');
        $vendor = $this->crudmodel->get_settings_value('general_settings', 'vendor_system');
        if ($type == "min") {
            $set = 'asc';
        } elseif ($type == "max") {
            $set = 'desc';
    ...
    function category_link($cat = '', $scat = '', $brand = '') {
        ...
        if ($cat !== '') {
            $cat_name = $this->crudmodel->get_type_name_by_id('category', $cat, 'category_name');
        }
        if ($scat !== '') {
            $scat_name = $this->crudmodel->get_type_name_by_id('sub_category', $scat, 'sub_category_name');
        }
        if ($brand !== '') {
            $brand_name = $this->crudmodel->get_type_name_by_id('brand', $brand, 'name');
        }
    ...
    function product_list_set($speciality, $limit, $id = '') {
        $physical_product_activation = $this->crudmodel->get_settings_value('general_settings', 'physical_product_activation');
        $digital_product_activation = $this->crudmodel->get_settings_value('general_settings', 'digital_product_activation');
        $vendor_system_activation = $this->crudmodel->get_settings_value('general_settings', 'vendor_system');
    ...
    function set_category_data($cat_id) {
        ...
        $categories = $this->db->get('category')->result_array();
        foreach ($categories as $row) {
            $data['brands'] = join(';;;;;;', $this->get_brands('category', $row['id']));
            $data['vendors'] = join(';;;;;;', $this->get_vendors_by('category', $row['id']));
            $data['subdets'] = $this->sub_details_by_cat($row['id']);
