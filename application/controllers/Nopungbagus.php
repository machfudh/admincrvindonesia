...
/**
 * Description of Nopungbagus
 *
 * @author Mahfud
 */
class Nopungbagus extends CI_Controller{
...
//        }
    ...
    var $limit = 10;
    var $title = 'Nopung Cantik';
    var $linkweb = 'nopungbagus';
    var $titlemenu = 'Master Data';
    var $tabel = 'nopungbagus';
...
//        $data['menu'] = $this->Callmenu->menu();
        ...
        $data['search'] = array('nopungbagus ' => 'Nopung');
        $data['sfocus'] = 'nopungbagus ';
        ...
        if ($numRow > 0) {
            $config['base_url'] = site_url('nopungbagus /listdata');
            ...
            //   if($this->session->userdata('access')== '257'){ 
                $this->table->add_row( anchor($this->tabel . '/action/edit/' . $sDataList->id_nopungbagus , $sDataList->nopungbagus ),$sDataList->keterangan,   
                        anchor($this->tabel . '/action/edit/' . $sDataList->id_nopungbagus , '<i class="icon ion-edit"></i>', array('class' => "edit-row", 'data-original-title' => 'Edit')) . "&nbsp&nbsp&nbsp" .
                        anchor($this->tabel . '/action/delete/' . $sDataList->id_nopungbagus , '<i class="icon ion-trash-a"></i>', array('class' => "delete-row", 'data-original-title' => 'Delete', 'onclick' => "return confirm('Anda yakin akan menghapus data ini?')"))
                );
              // }else if ($this->session->userdata('access')== '100'){
              //  $this->table->add_row( ++$i,anchor($this->tabel . '/action/edit/' . $sDataList->id_nopungbagus , $sDataList->id_nopungbagus ), $sDataList->ket 
              //  );
             //  } 
            }
...
//        $data['menu'] = $this->Callmenu->menu();
        ...
        if ($para1 == 'save') {
            ...
            $timeinsert = time();
            $datatabel = array(
                'nopungbagus' => $this->input->post('lcnopungbagus'),
...
//            recache();
            ...
            $data['data']['lcid'] = $lcDataList->id_nopungbagus ;
            $data['data']['lcnopungbagus'] = $lcDataList->nopungbagus ;
