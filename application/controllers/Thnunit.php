...
/**
 * Description of Thnunit
 *
 * @author Mahfud
 */
class Thnunit extends CI_Controller {
...
//        $data['menu'] = $this->Callmenu->menu();
        ...
        $data['search'] = array('thnunit' => 'Tahun Kendaraan');
        $data['sfocus'] = 'thnunit';
        ...
        if ($numRow > 0) {
            $config['base_url'] = site_url('thnunit/listdata');
            ...
            $this->table->set_heading(
                    array('data' => 'Tahun Kendaraan', 'style' => 'width:17%'), 
                    array('data' => 'Keterangan'), 
                    array('data' => '', 'style' => 'width:7%'));
            $i = 0 + $offset;
            foreach ($mDatalist as $sDataList) {
            //   if($this->session->userdata('access')== '257'){ 
                $this->table->add_row( anchor($this->tabel . '/action/edit/' . $sDataList->id_thnunit, $sDataList->tahun),$sDataList->keterangan,   
                        anchor($this->tabel . '/action/edit/' . $sDataList->id_thnunit, '<i class="icon ion-edit"></i>', array('class' => "edit-row", 'data-original-title' => 'Edit')) . "&nbsp&nbsp&nbsp" .
                        anchor($this->tabel . '/action/delete/' . $sDataList->id_thnunit, '<i class="icon ion-trash-a"></i>', array('class' => "delete-row", 'data-original-title' => 'Delete', 'onclick' => "return confirm('Anda yakin akan menghapus data ini?')"))
                );
              // }else if ($this->session->userdata('access')== '100'){
              //  $this->table->add_row( ++$i,anchor($this->tabel . '/action/edit/' . $sDataList->id_thnunit, $sDataList->id_thnunit), $sDataList->ket 
              //  );
             //  } 
            }
...
//            recache();
            ...
            $data['data']['lcid'] = $lcDataList->id_thnunit;
            $data['data']['lctahun'] = $lcDataList->tahun;
