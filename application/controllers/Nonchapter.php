...
/**
 * Description of Nonchapter
 *
 * @author Mahfud
 */
class Nonchapter extends CI_Controller {
...
//        }
    ...
    var $limit = 10;
    var $title = 'Non Chapter';
    var $linkweb = 'nonchapter';
    var $titlemenu = 'Master Data';
    var $tabel = 'nonchapter';
...
//        $data['menu'] = $this->Callmenu->menu();
        ...
        $data['search'] = array('nonchapter' => 'Kapasitas Mesin');
        $data['sfocus'] = 'nonchapter';
        ...
        if ($numRow > 0) {
            $config['base_url'] = site_url('nonchapter/listdata');
            ...
            foreach ($mDatalist as $sDataList) {
                ...
                $this->table->add_row(anchor($this->tabel . '/action/edit/' . $sDataList->id_nonchapter, $sDataList->namachapter), $sDataList->keterangan, 
                        anchor($this->tabel . '/action/edit/' . $sDataList->id_nonchapter, '<i class="icon ion-edit"></i>', array('class' => "edit-row", 'data-original-title' => 'Edit')) . "&nbsp&nbsp&nbsp" .
                        anchor($this->tabel . '/action/delete/' . $sDataList->id_nonchapter, '<i class="icon ion-trash-a"></i>', array('class' => "delete-row", 'data-original-title' => 'Delete', 'onclick' => "return confirm('Anda yakin akan menghapus data ini?')"))
                );
                // }else if ($this->session->userdata('access')== '100'){
                //  $this->table->add_row( ++$i,anchor($this->tabel . '/action/edit/' . $sDataList->id_nonchapter, $sDataList->id_nonchapter), $sDataList->ket 
                //  );
                //  } 
            }
...
//            recache();
            ...
            $data['data']['lcid'] = $lcDataList->id_nonchapter;
