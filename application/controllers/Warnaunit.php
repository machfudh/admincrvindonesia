...
/**
 * Description of Warnaunit
 *
 * @author Mahfud
 */
class Warnaunit extends CI_Controller {
...
//        }
    ...
    var $limit = 10;
    var $title = 'Warna Kendaraan';
    var $linkweb = 'warnaunit';
...
//        $data['menu'] = $this->Callmenu->menu();
        ...
        $data['search'] = array('warnaunit' => 'Warna Kendaraan');
        $data['sfocus'] = 'warnaunit';
        ...
        if ($numRow > 0) {
            $config['base_url'] = site_url('warnaunit/listdata');
            ...
            $this->table->set_heading(
                    array('data' => 'Warna Kendaraan', 'style' => 'width:17%'), 
                    array('data' => 'Keterangan'), 
                    array('data' => '', 'style' => 'width:7%'));
            $i = 0 + $offset;
            foreach ($mDatalist as $sDataList) {
            //   if($this->session->userdata('access')== '257'){ 
                $this->table->add_row( anchor($this->tabel . '/action/edit/' . $sDataList->id_warnaunit, $sDataList->warna),$sDataList->keterangan,   
                        anchor($this->tabel . '/action/edit/' . $sDataList->id_warnaunit, '<i class="icon ion-edit"></i>', array('class' => "edit-row", 'data-original-title' => 'Edit')) . "&nbsp&nbsp&nbsp" .
                        anchor($this->tabel . '/action/delete/' . $sDataList->id_warnaunit, '<i class="icon ion-trash-a"></i>', array('class' => "delete-row", 'data-original-title' => 'Delete', 'onclick' => "return confirm('Anda yakin akan menghapus data ini?')"))
                );
              // }else if ($this->session->userdata('access')== '100'){
              //  $this->table->add_row( ++$i,anchor($this->tabel . '/action/edit/' . $sDataList->id_warnaunit, $sDataList->id_warnaunit), $sDataList->ket 
              //  );
             //  } 
            }
...
//            recache();
            ...
            $data['data']['lcid'] = $lcDataList->id_warnaunit;
            $data['data']['lcwarna'] = $lcDataList->warna;
