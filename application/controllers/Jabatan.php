...
//        $data['menu'] = $this->Callmenu->menu();
        ...
        $data['search'] = array('jabatan' => 'Jabatan');
        $data['sfocus'] = 'jabatan';
        ...
        if ($numRow > 0) {
            $config['base_url'] = site_url('jabatan/listdata');
            ...
            $this->table->set_heading(
                    array('data' => 'Jabatan', 'style' => 'width:17%'), 
                    array('data' => 'Keterangan'),
                    array('data' => 'Terima Email', 'style' => 'width:17%'),
                    array('data' => 'Akes Data', 'style' => 'width:10%'),
                    array('data' => 'Rool Menu', 'style' => 'width:10%' ),
                    array('data' => '', 'style' => 'width:7%'));
            $i = 0 + $offset;
            foreach ($mDatalist as $sDataList) {
            //   if($this->session->userdata('access')== '257'){ 
                $this->table->add_row( anchor($this->tabel . '/action/edit/' . $sDataList->id_jabatan, $sDataList->nama),$sDataList->keterangan,$sDataList->memail=1?"Terima":"Tidak",$sDataList->akses>100?"Admin":"Operator",$sDataList->roolmenu,   
                        anchor($this->tabel . '/action/edit/' . $sDataList->id_jabatan, '<i class="icon ion-edit"></i>', array('class' => "edit-row", 'data-original-title' => 'Edit')) . "&nbsp&nbsp&nbsp" .
                        anchor($this->tabel . '/action/delete/' . $sDataList->id_jabatan, '<i class="icon ion-trash-a"></i>', array('class' => "delete-row", 'data-original-title' => 'Delete', 'onclick' => "return confirm('Anda yakin akan menghapus data ini?')"))
                );
              // }else if ($this->session->userdata('access')== '100'){
              //  $this->table->add_row( ++$i,anchor($this->tabel . '/action/edit/' . $sDataList->id_jabatan, $sDataList->id_jabatan), $sDataList->ket 
              //  );
             //  } 
            }
...
//        $data['menu'] = $this->Callmenu->menu();
        ...
        if ($para1 == 'save') {
            ...
            $datatabel = array(
                ...
                'idrool' => $this->input->post('lcidrool'),
                'memail' => $this->input->post('lcmemail'),
...
//            recache();
            ...
            $lcrool = $this->db->get('roolcrv')->result();
            $num_row = $this->db->get('roolcrv')->num_rows();
            if ($num_row > 0) {
             foreach ($lcrool as $row) {
                $data['listrool'][$row->id_roolcrv] = $row->nama;
                }
            }
            ...
            $data['data']['lcid'] = $lcDataList->id_jabatan;
            $data['data']['lcnama'] = $lcDataList->nama;
            $data['data']['lcketerangan'] = $lcDataList->keterangan;
            $data['data']['lcidrool'] = $lcDataList->idrool;
            $data['data']['lcmemail'] = "0";
